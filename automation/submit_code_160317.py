...
def main():
    ...
    print(f"==================================================")
    print(f"🔐 Submitting Code '{CODE_TO_ENTER}' for @{username}...")
    print(f"==================================================")
    ...
    with sync_playwright() as p:
        ...
        # Check if code entry box is already on page
        ...
        if not (code_input.count() > 0 and code_input.is_visible()):
        ...
        if code_input.count() > 0 and code_input.is_visible():
            print(f"Entering verification code {CODE_TO_ENTER}...")
        ...
        page.screenshot(path=os.path.join(BASE_DIR, "acc2_verification_result.png"))
        ...
        if create_icon.is_visible() or "instagram.com/direct/" in page.url or page.locator("svg[aria-label='Home']").count() > 0:
            print("✅ SUCCESS! Verification Complete & Session Saved!")
        else:
