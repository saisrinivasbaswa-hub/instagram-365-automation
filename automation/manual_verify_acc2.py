...
def main():
    ...
    print(f"==================================================")
    print(f"🌐 Launching Chrome Browser for @{username} Verification...")
    print(f"==================================================")
    ...
    with sync_playwright() as p:
        ...
        print("\n" + "🚀"*20)
        print("INSTAGRAM HAS SENT A NEW 6-DIGIT CODE TO YOUR EMAIL (b*******u@gmail.com).")
        print("PLEASE LOOK AT THE CHROME BROWSER WINDOW ON YOUR SCREEN, ENTER THE NEW CODE, AND CLICK 'CONTINUE'.")
        print("I am waiting for your login to complete...")
        print("🚀"*20 + "\n")

        # Monitoring loop: Wait for login success
        logged_in = False
        ...
        while time.time() - start_time < 300: # Wait up to 5 minutes
            ...
            if create_icon.is_visible() or "instagram.com/direct/" in page.url or page.locator("svg[aria-label='Home']").count() > 0:
                print("\n🎉 VERIFICATION SUCCESSFUL! Instagram session saved permanently!")
                logged_in = True
                break
        ...
        if not logged_in:
            print("Timeout waiting for manual verification.")
            browser.close()
            return

        # Automatically post Day 2 after verification!
